...
public class PublicPromptController {
    ...
    public ResponseEntity<PagedResponse<PromptSummaryResponse>> browsePrompts(
            ...
            @Parameter(description = "Sort order: newest | mostLiked | mostUsed | trending")
            @RequestParam(defaultValue = "newest") String sort,

            @Parameter(description = "Filter by category ID (includes subcategories)")
            @RequestParam(required = false) Long categoryId,

            @Parameter(description = "Filter by recommended AI entity ID")
            @RequestParam(required = false) Long aiEntityId,
